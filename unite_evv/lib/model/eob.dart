...
class ExplnationOfBenfits {
  ...
  List<BenefitEntry> filter(String claimType) {
    List<BenefitEntry> filtered = this
        .benefitEntries
        .where((benefitEntry) => benefitEntry.claimTypeCode == claimType)
        .toList();
...
class BenefitEntry {
  ...
  String
      claimTypeCode; //institutional(hospital),oral/dental,pharmacy/prescriptions,professional/office visits, vision
  ...
  List<BenefitEntryItem>
      benefitEntryItems; //a benefit entry can have multiple benefit entry items
  ...
  BenefitEntry(HuJSON entryJson) {
    ...
    List<dynamic> identifiersJson =
        entryJson.getArray(EOBJsonStatics.IDENTIFIER_PATH);
    if (identifiersJson != null) {
      this.buildIdentifiers(identifiersJson);
    }

    //organization and Facility
    this.organizationNPI =
        entryJson.getString(EOBJsonStatics.ORGANIZATION_NPI_PATH);
    this.facilityNPI = entryJson.getString(EOBJsonStatics.FACILITY_NPI_PATH);

    //billable period
    this.billablePeriodStart =
        entryJson.getString(EOBJsonStatics.BILLABLE_PERIOD_START_PATH);
    this.billablePeriodEnd =
        entryJson.getString(EOBJsonStatics.BILLABLE_PERIOD_END_PATH);

    //payment amount
    this.paymentAmount =
        entryJson.getDouble(EOBJsonStatics.PAYMENT_AMOUNT_PATH);

    //type descriptions
    List<dynamic> typeCodingsJSON =
        entryJson.getArray(EOBJsonStatics.TYPE_CODING_PATH);
    if (typeCodingsJSON != null) {
      this.buildTypeDescriptions(typeCodingsJSON);
    }

    //disgnosis Codes
    List<dynamic> disagnosisJSON =
        entryJson.getArray(EOBJsonStatics.DIAGNOSIS_PATH);
    if (disagnosisJSON != null) {
      this.buildDiagnosisCodes(disagnosisJSON);
    }

    //insurnace coverage reference
    this.insuranceCoverageReference =
        entryJson.getString(EOBJsonStatics.INSURANCE_COVERAGE_REFERENCE_PATH);

    //providers
    List<dynamic> careTeamArray =
        entryJson.getArray(EOBJsonStatics.CARE_TEAM_PATH);
    ...
    List<dynamic> resourceExtensionsJSON =
        entryJson.getArray(EOBJsonStatics.RESOURCE_EXTENSION_PATH);
    ...
    this.eobDate = this.claimTypeCode == EOBJsonStatics.PHARMACY
        ? benefitEntryItems[0].serviceDate
        : this.billablePeriodStart;
  }
  ...
  buildPaymentAmounts(List<dynamic> resourceExtensions) {
    for (Map<String, dynamic> resourceExtension in resourceExtensions) {
      ...
      String url = resourceExtensionJSON
          .getString(EOBJsonStatics.RESOURCE_EXTENSION_URL_PATH);
      double amount = resourceExtensionJSON
          .getDouble(EOBJsonStatics.RESOURCE_EXTENSION_AMOUNT_VALUE_PATH);
      switch (url) {
...
class Provider {
  ...
  Provider(HuJSON careTeamJson) {
    this.providerRole =
        careTeamJson.getString(EOBJsonStatics.CARE_TEAM_ROLE_PATH);
    this.providerQualification =
        careTeamJson.getString(EOBJsonStatics.CARE_TEAM_QUALIFICATION_PATH);
    this.providerNPI =
        careTeamJson.getString(EOBJsonStatics.CARE_TEAM_PROVIDER_NPI_PATH);
...
class BenefitEntryItem {
  ...
  BenefitEntryItem(HuJSON entryItemJson) {
    ...
    this.serviceCodeSystem =
        entryItemJson.getString(EOBJsonStatics.ITEM_SERVICE_CODE_SYSTEM_PATH);
    this.serviceCode =
        entryItemJson.getString(EOBJsonStatics.ITEM_SERVICE_CODE_PATH);
    this.serviceCodeDisplay =
        entryItemJson.getString(EOBJsonStatics.ITEM_SERVICE_CODE_DISPLAY_PATH);
    this.serviceDate =
        entryItemJson.getString(EOBJsonStatics.ITEM_SERVICE_DATE_PATH);
    this.servicePeriodStart =
        entryItemJson.getString(EOBJsonStatics.ITEM_SERVICE_PERIOD_START_PATH);
    this.servicePeriodEnd =
        entryItemJson.getString(EOBJsonStatics.ITEM_SERVICE_PERIOD_END_PATH);
    this.quantity = entryItemJson.getInt(EOBJsonStatics.ITEM_QUANTITY_PATH);

    //quantity extensions
    List<dynamic> quantityExtensions =
        entryItemJson.getArray(EOBJsonStatics.ITEM_QUANTITY_EXTENSION_PATH);
    if (quantityExtensions != null) {
      this.buildQuantityExtensions(quantityExtensions);
    }

    //adjudications
    List<dynamic> adjudicationsJSON =
        entryItemJson.getArray(EOBJsonStatics.ITEM_ADJUDICATION_PATH);
    this.adjudications = new List<Adjudication>();
    for (Map<String, dynamic> adjudicationJSON in adjudicationsJSON) {
      Adjudication adjudication =
          new Adjudication(new HuJSON(adjudicationJSON));
  ...
  buildQuantityExtensions(List<dynamic> quantityExtensions) {
    for (Map<String, dynamic> quantityExtension in quantityExtensions) {
      ...
      int quantity =
          quantityExtensionJSON.getInt(EOBJsonStatics.ITEM_VALUE_QUANTITY_PATH);
      switch (url) {
...
class DiagniosisCode {
  ...
  DiagniosisCode(HuJSON diagnosisCodingJSON) {
    this.code =
        diagnosisCodingJSON.getString(EOBJsonStatics.DIAGNOSIS_CODE_PATH);
    this.system = diagnosisCodingJSON
        .getString(EOBJsonStatics.DIAGNOSIS_CODE_SYSTEM_PATH);
    this.display = diagnosisCodingJSON
        .getString(EOBJsonStatics.DIAGNOSIS_CODE_DISPLAY_PATH);
  }
}
...
class Adjudication {
  ...
  Adjudication(HuJSON adjudicationJSON) {
    this.category = adjudicationJSON
        .getString(EOBJsonStatics.ITEM_ADJUDICATION_CATEGORY_CODING_PATH);
    this.reason = adjudicationJSON
        .getString(EOBJsonStatics.ITEM_ADJUDICATION_REASON_PATH);
    this.amount = adjudicationJSON
        .getDouble(EOBJsonStatics.ITEM_ADJUDICATION_AMOUNT_PATH);
  }
}
